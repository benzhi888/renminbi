...
#ifdef QT_GUI
const char *walletUrl = "https://www.renminbi.info/";
const char *walletDownloadsUrl = "https://www.renminbi.info/downloads/";
const char *forumsUrl = "http://www.renminbiforums.com";
bool fRestart = false;
...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\Renminbi
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\Renminbi
    // Mac: ~/Library/Application Support/Renminbi
    // Unix: ~/.renminbi
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "Renminbi";
#else
    fs::path pathRet;
...
{
    boost::filesystem::path pathPidFile(GetArg("-pid", "renminbid.pid"));
    ...
    if (vTimeOffsets.size() >= 5 && vTimeOffsets.size() % 2 == 1)
    {
        ...
        else
        {
            ...
            if (!fDone)
            {
                ...
                {
                    fDone = true;
                    string strMessage = _("Warning: Please check that your computer's date and time are correct! If your clock is wrong Renminbi will not work properly.");
                    strMiscWarning = strMessage;
                    printf("*** %s\n", strMessage.c_str());
                    uiInterface.ThreadSafeMessageBox(strMessage+" ", string("Renminbi"), CClientUIInterface::OK | CClientUIInterface::ICON_EXCLAMATION);
                }
            }
        }
        if (fDebug) {
