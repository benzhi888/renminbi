...
QString QRCodeDialog::getURI()
{
    ...
    if (ui->chkReqPayment->isChecked())
    {
        ...
        {
            // even if we allow a non RMB unit input in lnReqAmount, we generate the URI with RMB as unit (as defined in BIP21)
            ret += QString("?amount=%1").arg(BitcoinUnits::formatMaxDecimals(BitcoinUnits::RMB, ui->lnReqAmount->value(), BitcoinUnits::maxdecimals(BitcoinUnits::RMB)));
            paramCount++;
        }
        else
        {
